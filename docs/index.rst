...
On the one hand a end user can interact with the Jira board by manually
...
does add new packages to the Munki repository. These changes will be recognized
...
- Dry Run: The Munki Promoter can be run without commiting any changes.
    This enables easier debugging with a consistent state

- General command line interface (CLI): The Munki Promoter does support a simple
...
- Unit Tests: A majority of the code is covered by unit test to make sure future
    extensions do not break the intended functionality

- Logging: All kind of changes, differences and missing packages are logged to
